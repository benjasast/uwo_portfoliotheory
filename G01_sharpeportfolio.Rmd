...
We can see that the S&P 500 is well concentrated in the 0.0-0.05 range, with a slight positive skew. Reynolds and Hasbro both have a larger variance, but reynolds has fatter tails.
...
Not much we can say from this figure, except that all assets usually have negative returns, and that the S&P 500 seems to have a smaller variance.
...
The median return of reynolds is the largest, and seems to have a smaller variance than reynolds. The S&P has the lowest median return but the smallest variance in the interquantile range.
...
Reynolds seems to be the riskiest asset, as it has the largest standard deviation. Notice that is also the one with the highest holding period return, and mean return. Annualizing the returns only expands these differences.
...
Actual values for the returns on a yearly basis are very different from the annualized monthly values. Standard deviation is much higher, and year returns are higher. It is important to note that these calculations are made with very few data points.
...
Table including the portfolio returns
```{r}
kable(final_returns, digits = 3, col.names = col)
```

The two portfolios S&P 500 + reynolds and hasbro have almost all the same properties in terms of mean return and sd. Is only after annualizing that we can see that SP500 + reynolds has a slightly smaller variance. Why is this so? Let's check the correlations.

```{r, error=FALSE, warning=FALSE}
cor(df$`S&P`,df$HASBRO)
cor(df$`S&P`,df$REYNOLDS)
```
Lower correlation of SP500 and reynolds.
...
The beta of each asset is an indicator of the expected payment that should be expected for taking the inherent risk of the asset. It is interesting to see that the higher the correlation the higher the beta is.
